...
type BroadcastState = {
  step: 'menu' | 'text' | 'button' | 'image' | 'preview';
  text: string;
  buttonText?: string;
  buttonUrl?: string;
  imageFileId?: string;
...
function broadcastMenuKeyboard(): Record<string, unknown> {
  return {
    inline_keyboard: [
      [{ text: "📝 Добавить текст", callback_data: "bcast_add_text" }],
...
async function showBroadcastPreview(token: string, chatId: number) {
  ...
  if (state.text) {
    ...
    previewText += "<i>(текст не добавлен)</i>";
  }
  ...
  previewText += "\n\n─".repeat(30) + "\n\n";
  previewText += "📋 <b>Что добавлено:</b>\n";
  previewText += state.text ? "✅ Текст\n" : "❌ Текст\n";
  previewText += state.buttonText ? "✅ Кнопка\n" : "❌ Кнопка\n";
  previewText += state.imageFileId ? "✅ Изображение\n" : "❌ Изображение\n";

  const keyboard = {
    inline_keyboard: [
      [{ text: "📝 Редактировать текст", callback_data: "bcast_edit_text" }],
      [{ text: "🔗 Редактировать кнопку", callback_data: "bcast_edit_button" }],
      [{ text: "🖼️ Редактировать изображение", callback_data: "bcast_edit_image" }],
      [
        { text: "✅ Опубликовать", callback_data: "bcast_publish" },
        { text: "❌ Отменить", callback_data: "bcast_cancel" }
      ]
...
async function handleBroadcastCallback(token: string, chatId: number, data: string) {
  ...
  if (data === "bcast_menu") {
    await sendMessage(token, chatId, 
      ...
      "Выберите, что хотите добавить в сообщение:",
      broadcastMenuKeyboard()
    );
    return;
  }

  if (data === "bcast_add_text") {
    state.step = 'text';
    await sendMessage(token, chatId,
  ...
  if (data === "bcast_add_button") {
    state.step = 'button';
    await sendMessage(token, chatId,
      "🔗 <b>Введите текст и ссылку для кнопки</b>\n\n" +
      "Формат:\n" +
  ...
  if (data === "bcast_publish") {
    if (!state.text) {
      await sendMessage(token, chatId, "❌ Добавьте текст сообщения!");
      return;
    }
...
async function handleCommand(token: string, chatId: number, text: string): Promise<void> {
  ...
  if (command === "/broadcast") {
    ...
    // Создаем состояние и показываем меню
    broadcastStates.set(chatId, { step: 'menu', text: '' });
    await sendMessage(token, chatId, 
      "📢 <b>Создание рассылки</b>\n\n" +
      "Выберите, что хотите добавить в сообщение:",
      broadcastMenuKeyboard()
    ...
    try {
      ...
      for (const update of updates) {
        ...
        if (update.callback_query) {
          ...
          // Если это изображение для broadcast
          if (update.message.photo && state && state.step === 'image') {
