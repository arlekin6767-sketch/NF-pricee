  ...
  return {
    inline_keyboard: [
      ...
      [
        ...
        { text: "🗑️ Удалить кнопку", callback_data: "bcast_remove_buttons" }
      ],
      [
        { text: "🖼️ Добавить изображение", callback_data: "bcast_add_image" },
        { text: "👁️ Посмотреть превью", callback_data: "bcast_preview" }
      ],
      [
...
function controlPanelKeyboard(): Record<string, unknown> {
  return {
    inline_keyboard: [
      [
        { text: "👁️ Предпросмотр", callback_data: "bcast_preview" },
        { text: "📤 Опубликовать", callback_data: "bcast_publish" }
      ],
      [
        { text: "📝 Редактировать", callback_data: "bcast_edit" },
        { text: "🔄 Очистить всё", callback_data: "bcast_clear" }
      ],
      [
...
async function showBroadcastStatus(token: string, chatId: number) {
  ...
  statusText += "🔗 <b>Кнопка:</b>\n";
  if (state.buttonText && state.buttonUrl) {
    statusText += `✅ ${state.buttonText}\n`;
  } else {
    statusText += "❌ Не добавлена\n";
  }
  statusText += "\n";
  ...
  if (state.text) {
    statusText += "✅ Сообщение готово к просмотру!\n";
    statusText += "Нажмите «Посмотреть превью»";
  } else {
...
async function showBroadcastPreview(token: string, chatId: number) {
  ...
  const previewKeyboard = {
    inline_keyboard: [
      [
        ...
        { text: "🔗 Редактировать кнопку", callback_data: "bcast_edit_button" }
      ],
      [
        { text: "🖼️ Изменить изображение", callback_data: "bcast_edit_image" },
        { text: "📤 Опубликовать", callback_data: "bcast_publish" }
      ],
      [
        { text: "🔙 Назад", callback_data: "bcast_back" }
  ...
  previewCaption += "\n\n━━━━━━━━━━━━━━━━━━━\n\n";
  previewCaption += "✅ Всё верно? Нажмите «Опубликовать»";
...
async function publishBroadcast(token: string, chatId: number) {
  const state = broadcastStates.get(chatId);
  if (!state) {
    await sendMessage(token, chatId, "❌ Рассылка не найдена");
    return;
  }
  ...
  if (!state.previewSent) {
    ...
    await sendMessage(token, chatId,
      ...
      "Нажмите «Сначала посмотреть» или «Всё равно опубликовать».",
      confirmKeyboard
    );
    return;
  }
...
async function handleBroadcastCallback(token: string, chatId: number, data: string) {
  ...
  if (data === "bcast_edit_button") {
    state.buttonText = undefined;
    state.buttonUrl = undefined;
    state.previewSent = false;
    await sendMessage(token, chatId,
      "✏️ <b>Введите новую кнопку</b>\n\n" +
  ...
  if (data === "bcast_remove_buttons") {
    ...
    state.buttonUrl = undefined;
    state.previewSent = false;
    await sendMessage(token, chatId, "✅ Кнопка удалена!");
  ...
  if (data === "bcast_publish") {
    await publishBroadcast(token, chatId);
    return;
  }
  ...
  if (data === "bcast_clear") {
    broadcastStates.delete(chatId);
    await sendMessage(token, chatId, "🔄 Всё очищено. Начните заново с /broadcast");
    return;
  }
