...
type AdminState = { step: "text" | "photo" | "button" | null; draft: BroadcastDraft };
...
const adminChatId = process.env.ADMIN_CHAT_ID
  ...
  : undefined;
const BROADCAST_DELAY_MS = 1_000;
...
function builderKeyboard(draft: BroadcastDraft): Record<string, unknown> {
  const rows: Array<Array<Record<string, string>>> = [
    [
      { text: draft.text ? "✏️ Изменить текст" : "📝 Добавить текст", callback_data: "bc_text" },
      { text: draft.photoUrl ? "🖼 Изменить фото" : "🖼 Добавить фото", callback_data: "bc_photo" },
    ],
    [
      { text: "🔗 Добавить кнопку", callback_data: "bc_button" },
    ],
  ];
  if (draft.text || draft.photoUrl) {
    rows.push([{ text: "📨 Отправить рассылку", callback_data: "bc_send" }]);
  }
  rows.push([{ text: "❌ Отмена", callback_data: "bc_cancel" }]);
  return { inline_keyboard: rows };
}

function builderText(draft: BroadcastDraft): string {
  const lines = ["🛠 <b>Конструктор рассылки</b>", ""];
  lines.push(draft.text
    ? `📝 Текст: <i>${html(draft.text.slice(0, 60))}${draft.text.length > 60 ? "..." : ""}</i>`
    : "📝 Текст: —");
  lines.push(draft.photoUrl ? "🖼 Фото: ✅ добавлено" : "🖼 Фото: —");
  if (draft.buttons.length) {
    lines.push(`🔗 Кнопки: ${draft.buttons.map((b) => html(b.text)).join(", ")}`);
  } else {
    lines.push("🔗 Кнопки: —");
  }
  return lines.join("\n");
...
function draftReplyMarkup(draft: BroadcastDraft): Record<string, unknown> | undefined {
  if (!draft.buttons.length) return undefined;
  return { inline_keyboard: [draft.buttons.map((b) => ({ text: b.text, url: b.url }))] };
}

async function broadcast(token: string, draft: BroadcastDraft): Promise<void> {
  const replyMarkup = draftReplyMarkup(draft);
  let sent = 0;
  let failed = 0;
  for (const chatId of subscribers) {
    try {
      if (draft.photoUrl) {
        await sendPhoto(token, chatId, draft.photoUrl, draft.text ?? "", replyMarkup);
      } else if (draft.text) {
        await sendMessage(token, chatId, draft.text, replyMarkup);
      }
      sent += 1;
...
async function handleCommand(token: string, chatId: number, text: string): Promise<void> {
  ...
  if (chatId === adminChatId && !text.startsWith("/")) {
    const state = adminStates.get(chatId);
    if (state?.step) {
      if (state.step === "text") {
        state.draft.text = text.trim();
        state.step = null;
        await sendMessage(token, chatId, `✅ Текст сохранён.\n\n${builderText(state.draft)}`, builderKeyboard(state.draft));
      } else if (state.step === "photo") {
        const url = text.trim();
        if (!url.startsWith("http")) {
          await sendMessage(token, chatId, "⚠️ Отправь прямую ссылку на изображение (https://...)");
          return;
        }
        state.draft.photoUrl = url;
        state.step = null;
        await sendMessage(token, chatId, `✅ Фото добавлено.\n\n${builderText(state.draft)}`, builderKeyboard(state.draft));
      } else if (state.step === "button") {
        const match = text.match(/^(.+?)\|(https?:\/\/.+)$/);
        if (!match) {
          await sendMessage(token, chatId, "⚠️ Неверный формат. Попробуй ещё раз:\n<code>Название кнопки|https://url.com</code>");
          return;
        }
        state.draft.buttons.push({ text: match[1].trim(), url: match[2].trim() });
        state.step = null;
  ...
  if (command === "/start") {
    ...
    await sendMessage(token, chatId, `Подписчиков: ${subscribers.size}`);
  } else if (command === "/broadcast") {
    if (chatId !== adminChatId) {
      await sendMessage(token, chatId, "Недостаточно прав.");
      return;
    }
    const draft: BroadcastDraft = { buttons: [] };
    adminStates.set(chatId, { step: null, draft });
    await sendMessage(token, chatId, builderText(draft), builderKeyboard(draft));
...
async function handleCallback(token: string, callback: TelegramCallbackQuery): Promise<void> {
  ...
  if (data === "bc_text" || data === "bc_photo" || data === "bc_button") {
    if (chatId !== adminChatId) return;
    const state = adminStates.get(chatId) ?? { step: null, draft: { buttons: [] } };
    if (data === "bc_text") {
      state.step = "text";
      adminStates.set(chatId, state);
      await sendMessage(token, chatId, "📝 Введи текст рассылки:");
    } else if (data === "bc_photo") {
      state.step = "photo";
      adminStates.set(chatId, state);
      await sendMessage(token, chatId, "🖼 Отправь прямую ссылку на фото (https://...):");
    } else if (data === "bc_button") {
      state.step = "button";
      adminStates.set(chatId, state);
      await sendMessage(token, chatId, "🔗 Введи кнопку в формате:\n<code>Название кнопки|https://url.com</code>");
    }
    return;
  }
  ...
  if (data === "bc_send") {
    if (chatId !== adminChatId) return;
    const state = adminStates.get(chatId);
    if (!state?.draft.text && !state?.draft.photoUrl) {
      await sendMessage(token, chatId, "⚠️ Добавь хотя бы текст или фото перед отправкой.");
      return;
    }
    const draft = state.draft;
    adminStates.delete(chatId);
    await sendMessage(token, chatId, `📨 Начинаю рассылку на ${subscribers.size} пользователей...`);
    void broadcast(token, draft);
    return;
  }
