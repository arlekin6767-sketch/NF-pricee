...
async function broadcast(token: string, text: string): Promise<void> {
  let sent = 0;
  let failed = 0;
...
async function handleCommand(token: string, chatId: number, text: string): Promise<void> {
  ...
  if (command === "/start") {
    ...
    if (!args) {
      await sendMessage(token, chatId, "Формат: <code>/broadcast Текст сообщения</code>");
      return;
    }
    await sendMessage(token, chatId, `Начинаю рассылку на ${subscribers.size} пользователей...`);
    void broadcast(token, args);
