...
const REQUESTS_BEFORE_PARTNER_GATE = 3;
const PARTNER_BOT_URL = "https://t.me/GiftsBattle_bot?startapp=ref_Bh7NLRnaU";
...
function partnerGateKeyboard(): Record<string, unknown> {
  return {
    inline_keyboard: [
      [{ text: "Открыть GiftsBattle", url: PARTNER_BOT_URL }],
...
function partnerGateText(): string {
  return [
    ...
    "",
    "Ты использовал 3 бесплатных проверки.",
    "Чтобы продолжить пользоваться оценкой NFT-подарков:",
    "",
    "1. Открой бота GiftsBattle по кнопке ниже",
    "2. Подпишись и вернись сюда",
    "3. Нажми «Я подписался — продолжить»",
    "",
    "После этого проверки снова станут доступны.",
  ].join("\n");
}
